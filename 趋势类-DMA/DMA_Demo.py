# -*- coding: utf-8 -*-
#通达信DMA平行线差指标公式：平行线差（趋势类-DMA）指标是利用两条不同期间的平均线，来判断当前买卖能量的大小和未来价格趋势。DMA指标是一种中短期指标。
#DMA平行线差指标计算公式编辑： 趋势类-DMA=股价短期平均值—股价长期平均值 AMA=DMA短期平均值.
...
"""
DMA指标用法
1.趋势类-DMA 向上交叉其平均线时，买进；
2.趋势类-DMA 向下交叉其平均线时，卖出；
3.趋势类-DMA 的交叉信号比MACD、TRIX 略快；
4.趋势类-DMA 与股价产生背离时的交叉信号，可信度较高；
5.趋势类-DMA、MACD、TRIX 三者构成一组指标群，互相验证。
"""
