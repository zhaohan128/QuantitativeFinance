...
#计算LMA1、LMA2的值。
def calc_LMA(mkt_data, N1=5, N2=90):
    """
    计算MA1、MA2的值
    LMA1:=MA(LOW,N1);
    LMA2:=MA(LOW,N2);

    :param mkt_data DataFrame 股票历史行情数据，日维度，需要包含最低价['low']
    :param N1 int LMA1参数N1
    :param N2 int LMA2参数N2

    :return mkt_data DataFrame 新增2列,LMA1、LMA2的值
    """

    mkt_data['LMA1'] = mkt_data['low'].rolling(N1).mean()
    mkt_data['LMA2'] = mkt_data['low'].rolling(N2).mean()
...
def visualize_performance(mkt_data):
    ...
    # 绘制指标
    indi.line(dt, mkt_data['LMA1'], line_width=1, color='yellow')
    indi.line(dt, mkt_data['LMA2'], line_width=1, color='green')
