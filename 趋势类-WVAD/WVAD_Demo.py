...
#计算WVAD 和 MAWVAD的值。
def calc_WVAD(mkt_data, n=24, m=6):
    """
    计算WVAD 和 MAWVAD的值。
    指标源代码
    (CLOSE-OPEN)/(HIGH-LOW)*VOL
    ...
    :param mkt_data DataFrame 股票历史行情数据，日维度，需要包含收盘价['close']、开盘价['open']、最高价['high']、最低价['low']、成交量['volume']
    :param n int VPT参数n
    ...
    mkt_data['WVAD'] = (mkt_data['volume'] * (mkt_data['close']-mkt_data['open']) / (mkt_data['high'] - mkt_data['low'])).rolling(n).sum()
    mkt_data['MAWVAD'] = mkt_data['WVAD'].rolling(m).mean()
...
def calc_signal(mkt_data):
    ...
    """
    WVAD = mkt_data['WVAD']
    MAWVAD = mkt_data['MAWVAD']

    """ 计算信号 """
    signals = []
    for wvad, pre_wvad in zip(WVAD, WVAD.shift(1)):
        signal = None
        if (wvad > 0) & (pre_wvad < 0):
            signal = 1
        elif (wvad < 0) & (pre_wvad > 0):
            signal = -1
...
def visualize_performance(mkt_data):
    ...
    WVAD = mkt_data['WVAD']
    MAWVAD = mkt_data['MAWVAD']
    indi.line(dt, WVAD, line_width=1, color='yellow')
    indi.line(dt, MAWVAD, line_width=1, color='blue')
