...
#计算MA1、MA2的值。
def calc_MA(mkt_data, N1=5, N2=90):
    """
    计算MA1、MA2的值
    MA1:=MA(CLOSE,N1);
    MA2:=MA(CLOSE,N2);
    ...
    :return mkt_data DataFrame 新增2列,MA1、MA2的值
    """

    mkt_data['MA1'] = mkt_data['close'].rolling(N1).mean()
    mkt_data['MA2'] = mkt_data['close'].rolling(N2).mean()
...
def visualize_performance(mkt_data):
    ...
    # 绘制指标
    indi.line(dt, mkt_data['MA1'], line_width=1, color='yellow')
    indi.line(dt, mkt_data['MA2'], line_width=1, color='green')
