...
#计算HMA1、HMA2的值。
def calc_HMA(mkt_data, N1=5, N2=90):
    """
    计算MA1、MA2的值
    MA1:=MA(HIGH,N1);
    MA2:=MA(HIGH,N2);

    :param mkt_data DataFrame 股票历史行情数据，日维度，需要包含最高价['high']
    :param N1 int MA1参数N1
    :param N2 int MA2参数N2

    :return mkt_data DataFrame 新增2列,HMA1、HMA2的值
    """

    mkt_data['HMA1'] = mkt_data['high'].rolling(N1).mean()
    mkt_data['HMA2'] = mkt_data['high'].rolling(N2).mean()
...
def visualize_performance(mkt_data):
    ...
    # 绘制指标
    indi.line(dt, mkt_data['HMA1'], line_width=1, color='yellow')
    indi.line(dt, mkt_data['HMA2'], line_width=1, color='green')
